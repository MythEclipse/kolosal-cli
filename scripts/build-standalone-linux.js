...
const _NODE_MAJOR = NODE_VERSION.split('.')[0].substring(1); // e.g., "20"
...
async function buildStandalone() {
  console.log(
    '🔨 Building standalone Linux package (with embedded Node.js)...\n',
  );
...
async function createDeb(appDir) {
  ...
  await fs.symlink(
    '/opt/kolosal-app/bin/kolosal',
    path.join(binDir, 'kolosal'),
  );
  ...
  try {
    ...
    if (deps.packages && deps.packages.length > 0) {
      ...
      console.log(
        `   Using ${deps.packages.length} package dependencies from analysis`,
      );
    }
  } catch (error) {
...
`;
  ...
  // Build the package
  const debOutput = path.join(
    distDir,
    `kolosal-code_${pkg.version}_${arch}.deb`,
  );
  ...
  try {
    ...
    console.warn(
      '⚠️  Could not read package info (dpkg-deb might not be installed)',
    );
  }
...
async function createRpm(appDir) {
  ...
  try {
    ...
    if (deps.packages && deps.packages.length > 0) {
      ...
      console.log(
        `   Using ${rpmPackages.length} package dependencies for RPM`,
      );
    }
  } catch (error) {
...
`;
  ...
  // Build the RPM
  const rpmOutput = path.join(
    rpmsDir,
    arch,
    `kolosal-code-${pkg.version}-1.${arch}.rpm`,
  );
...
async function createTarball(appDir) {
  ...
  const tarballPath = path.join(
    distDir,
    `kolosal-code-${pkg.version}-linux-${arch}.tar.gz`,
  );
  ...
  console.log(
    '   sudo ln -s /opt/kolosal-app/bin/kolosal /usr/local/bin/kolosal',
  );
