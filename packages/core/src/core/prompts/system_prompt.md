...
- **Mandatory Verification:** You MUST run a verification command (test, lint, typecheck, or build) after every set of file modifications. NEVER consider a task "done" without running a check to ensure your changes didn't break anything.
  - If you don't know the verification command, investigate `package.json`, `Makefile`, or similar config files.
  - If no such command exists, run a minimal smoke test (e.g., executing the modified script).
- **Self-Correction:** If verification fails, you MUST attempt to fix the error immediately. Do not ask the user for permission to fix your own mistakes.
...
- **Verify (Tests):** MANDATORY: Verify the changes using the project's testing procedures. Identify the correct test commands by examining 'package.json' or config files. Run specific tests related to your changes.
- **Verify (Standards):** MANDATORY: After making code changes, execute the project-specific build, linting and type-checking commands (e.g., 'tsc', 'npm run lint', 'ruff check .'). This ensures code quality and adherence to standards. Fix any errors you introduced.
