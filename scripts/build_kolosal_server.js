...
async function buildKolosalServer() {
  ...
  const _releaseDir = path.join(buildDir, 'Release');
  ...
  try {
    ...
    throw new Error(
      '❌ kolosal-server directory not found. Make sure the kolosal-server subproject exists.',
    );
  }
...
async function buildDualInferenceEngines() {
  ...
  try {
    await execAsync(
      'cmake .. -DCMAKE_BUILD_TYPE=Release -DUSE_CUDA=OFF -DUSE_VULKAN=OFF -DUSE_METAL=OFF',
      {
        cwd: buildDirCpu,
        stdio: 'inherit',
      },
  ...
  try {
    await execAsync(
      'cmake .. -DCMAKE_BUILD_TYPE=Release -DUSE_CUDA=OFF -DUSE_VULKAN=ON -DUSE_METAL=OFF',
      {
        cwd: buildDirVulkan,
        stdio: 'inherit',
      },
  ...
  const vulkanFiles = ['libllama-vulkan.so'];
...
async function copyToDistribution(sourceData) {
  ...
  console.log(
    isDualBuild
      ? '🔥 Processing dual-build artifacts...'
      : '📋 Processing single-build artifacts...',
  );

  // Files to copy to bin directory
  const executableFiles = ['kolosal-server'];
  ...
  if (isDualBuild) {
    ...
    for (const file of cpuLibraries) {
      ...
      try {
        ...
        throw new Error(
          `❌ Failed to copy CPU library ${file}: ${error.message}`,
        );
      }
    }
    ...
    if (sourceData.vulkan) {
      ...
      for (const file of vulkanLibraries) {
        ...
        try {
          ...
          console.warn(
            `⚠️  Failed to copy Vulkan library ${file}: ${error.message}`,
          );
        }
      }
    } else {
      console.warn(
        '   ⚠️  Vulkan build not available, skipping Vulkan libraries',
      );
    }
  } else {
    // Single build - copy libraries as before
    const libraryFiles = [`libkolosal_server${libExtension}`, inferenceLibName];
  ...
  if (isMac) {
    const macConfigTemplate = path.join(
      rootDir,
      'kolosal-server',
      'configs',
      'config.macos.yaml',
    ...
    const linuxConfigTemplate = path.join(
      rootDir,
      'kolosal-server',
      'configs',
      'config.linux.yaml',
...
async function testKolosalServer(binDir, libDir, resourcesDir) {
  ...
  if (libDir) {
    env[libEnvKey] =
      libDir + (env[libEnvKey] ? `${delimiter}${env[libEnvKey]}` : '');
  }
  ...
  if (process.env.KOLOSAL_SERVER_TEST_ARGS) {
    args.splice(
      0,
      args.length,
      ...process.env.KOLOSAL_SERVER_TEST_ARGS.split(' ').filter(Boolean),
    );
    console.log(
      `   Using KOLOSAL_SERVER_TEST_ARGS override: ${args.join(' ')}`,
    );
  }
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      child.on('close', (code, signal) => {
        ...
        if (timedOut) {
          return reject(
            new Error(
              `Timed out after ${TEST_TIMEOUT_MS}ms (signal: ${signal ?? 'N/A'})`,
            ),
          );
        }
        if (code !== 0) {
          return reject(
            new Error(
              `Exited with code ${code ?? 'unknown'}; stderr: ${stderr.trim()}`,
            ),
          );
        }
        resolve();
    ...
    if (stdout.trim()) {
      ...
      console.log(
        `   Output: ${preview}${stdout.trim().length > 160 ? '…' : ''}`,
      );
    }
  } catch (error) {
    console.warn(`⚠️  Could not test kolosal-server: ${error.message}`);
    console.warn(
      '   This might be normal if the server requires specific arguments',
    );
  }
}
...
async function main() {
  try {
    console.log(
      '🚀 Starting kolosal-server build and integration process...\n',
    );
    ...
    if (isLinux) {
      ...
      console.log(
        `     - ${path.join(libDir, `libkolosal_server${libExtension}`)}`,
      );
      ...
      console.log(
        `   • Libraries: ${path.join(libDir, `libkolosal_server${libExtension}`)}, ${path.join(libDir, inferenceLibName)}`,
      );
    }
